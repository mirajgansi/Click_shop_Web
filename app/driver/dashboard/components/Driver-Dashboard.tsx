...
export default function DriverDashboardPage({ user }: { user: any }) {
  ...
  const [driverName, setDriverName] = useState(user?.name || user?.username || "Driver");
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (ordersRes?.success === false) throw new Error(ordersRes.message || "Failed to load assigned orders");

        const odata = ordersRes?.data ?? ordersRes;
        const meta = odata?.pagination || odata?.meta || {};
        const list = odata?.orders || odata?.items || (Array.isArray(odata) ? odata : []) || [];

        setAssigned(Number(meta?.total ?? odata?.total ?? list.length ?? 0));
        setDelivered(list.filter((o: any) => o.status === "delivered").length);

        const getOrderTime = (o: any) => new Date(o.updatedAt || o.createdAt || 0).getTime();
        ...
        if (lastOrder) {
          ...
          else if (lastOrder.status === "shipped" || lastOrder.status === "pending") setActivityType("Assigned/Updated");
